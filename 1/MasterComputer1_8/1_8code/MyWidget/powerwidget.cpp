...
{
    m_data.PulseWidethString = PulseWidthEidt->text();
    m_data.PulseRateString = PulseRateEdit->text();
    m_data.PowerString = PowerEdit->text();
    emit SendPowerData(m_data);
}

void PowerWidget::HideButton()
{
    setOkButton->setEnabled(false);
}

void PowerWidget::ShowButton()
{
    setOkButton->setEnabled(true);
}
