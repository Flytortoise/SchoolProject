...
void RunWidget::HandleRunData()
{
    int PositionIndex = 0;
    int SpeedIndex = 0;
    RunData.xData[RunToHandleData::ShowPositon] = xLocationEdit->text();
    RunData.xData[RunToHandleData::SetPositon] = setEdit[PositionIndex++]->text();
    RunData.xData[RunToHandleData::SetSpeed] = speedEdit[SpeedIndex++]->text();
    m_OrgData[XIndex] = RunData.xData[RunToHandleData::ShowPositon].toDouble();

    RunData.yData[RunToHandleData::ShowPositon] = yLocationEdit->text();
    RunData.yData[RunToHandleData::SetPositon] = setEdit[PositionIndex++]->text();
    RunData.yData[RunToHandleData::SetSpeed] = speedEdit[SpeedIndex++]->text();
    m_OrgData[YIndex] = RunData.yData[RunToHandleData::ShowPositon].toDouble();

    RunData.zData[RunToHandleData::ShowPositon] = zLocationEdit->text();
    RunData.zData[RunToHandleData::SetPositon] = setEdit[PositionIndex++]->text();
    RunData.zData[RunToHandleData::SetSpeed] = speedEdit[SpeedIndex++]->text();
    m_OrgData[ZIndex] = RunData.zData[RunToHandleData::ShowPositon].toDouble();

    RunData.pztData[RunToHandleData::ShowPositon] = pztLocationEdit->text();
    RunData.pztData[RunToHandleData::SetPositon] = setEdit[PositionIndex]->text();
    m_OrgData[PZTIndex] = RunData.pztData[RunToHandleData::ShowPositon].toDouble();

    emit SendRunData();
}

void RunWidget::GetRecvData(ComToRunData comrunData)
{
    if(comrunData.isOk)
    {
    ...
    //X
    if(comrunData.isPlus[ComToRunData::XIndex])
        xLocationEdit->setText(QString::number(m_OrgData[XIndex] + comrunData.PositionData[ComToRunData::XIndex],
                               10, 3)); //10进制，小数点3位
    else
        xLocationEdit->setText(QString::number(m_OrgData[XIndex] - comrunData.PositionData[ComToRunData::XIndex],
                               10, 3));
    ...
    //Y
    if(comrunData.isPlus[ComToRunData::YIndex])
        yLocationEdit->setText(QString::number(m_OrgData[YIndex] + comrunData.PositionData[ComToRunData::YIndex],
                               10, 3));
    else
        yLocationEdit->setText(QString::number(m_OrgData[YIndex] - comrunData.PositionData[ComToRunData::YIndex],
                               10, 3));
    ...
    //Z
    if(comrunData.isPlus[ComToRunData::ZIndex])
        zLocationEdit->setText(QString::number(m_OrgData[ZIndex] + comrunData.PositionData[ComToRunData::ZIndex],
                               10, 3));
    else
        zLocationEdit->setText(QString::number(m_OrgData[ZIndex] - comrunData.PositionData[ComToRunData::ZIndex],
                               10, 3));
    //PZT
    pztLocationEdit->setText(QString::number(m_OrgData[PZTIndex] + comrunData.PositionData[ComToRunData::PZTIndex],
                           10, 3));
